...
object MD5Security{
    ...
    @Volatile private var encryptedText: String? = null
    ...
    fun encryptPassword(password: String): String?{
        ...
        digestBytes = messageDigest?.digest(password.toByteArray())
        bigInteger = BigInteger(1, digestBytes)
        encryptedText = bigInteger?.toString(128)
        while (encryptedText?.length!! < 256){
...
fun Routing.dataRouting(){
    ...
    post("/insert"){
        ...
        //TODO("get pass, encrypt, save in DB, read all data show encrypted pass,  validate password")

        try {
            val affectedRows = getDatabase()?.insert(LoginTable){
                set(LoginTable.id,login.id)
                set(LoginTable.username,login.username)
                set(LoginTable.password,MD5Security.encryptPassword(login.password))
            }
    ...
    post("/validate"){
        ...
        val loginResponse = getDatabase()?.from(LoginTable)?.select()?.where{ LoginTable.id eq login.id }?.map {
            Login(
                ...
                password = if (MD5Security.encryptPassword(login.password) == it[LoginTable.password]!!) login.password else it[LoginTable.password]!!
            )
        }?.firstOrNull()
