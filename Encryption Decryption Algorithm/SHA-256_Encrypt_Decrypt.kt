    ...
    post("/insert"){
        ...
        try {
            val affectedRows = getDatabase()?.insert(LoginTable){
                ...
                set(LoginTable.password,SHA256Security.encryptPassword(login.password))
            }
    ...
    post("/validate"){
        ...
        val loginResponse = getDatabase()?.from(LoginTable)?.select()?.where{ LoginTable.id eq login.id }?.map {
            Login(
                ...
                password = if (SHA256Security.encryptPassword(login.password) == it[LoginTable.password]!!) login.password else it[LoginTable.password]!!
            )
        }?.firstOrNull()
